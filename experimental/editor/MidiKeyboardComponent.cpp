...
void MidiKeyboardComponent::getKeyPosition (int midiNoteNumber, const float keyWidth_, int& x, int& w) const {
    ...
    static const float notePos[] = { 0.0f, 1 - blackNoteWidth * 0.6f, 1.0f, 2 - blackNoteWidth * 0.4f, 2.0f, 3.0f, 4 - blackNoteWidth * 0.7f, 4.0f, 5 - blackNoteWidth * 0.5f, 5.0f, 6 - blackNoteWidth * 0.3f, 6.0f };
...
int MidiKeyboardComponent::findNoteAt (Point<int> pos) {
    Point<int> p (pos);
    ...
    for (octave = 0; octave < 128; octave += 12) {
        for (int white = 0; white < 7; ++white) {
            ...
            if (noteNum >= rangeStart && noteNum <= rangeEnd) {
                ...
                drawWhiteNote (noteNum, g, pos.getX(), pos.getY(), pos.getWidth(), pos.getHeight(), state.isNoteOnForChannels (midiInChannelMask, noteNum), mouseOverNotes.contains (noteNum), lineColour, textColour);
            }
        }
    }
    ...
    for (octave = 0; octave < 128; octave += 12) {
        for (int black = 0; black < 5; ++black) {
            ...
            if (noteNum >= rangeStart && noteNum <= rangeEnd) {
                ...
                drawBlackNote (noteNum, g, pos.getX(), pos.getY(), pos.getWidth(), pos.getHeight(), state.isNoteOnForChannels (midiInChannelMask, noteNum), mouseOverNotes.contains (noteNum), blackNoteColour);
            }
        }
    }
}
        ...
        if (newNote != oldNoteDown) {
            if (oldNoteDown >= 0) {
                ...
                if (! mouseDownNotes.contains (oldNoteDown))
                    state.noteOff (midiChannel, oldNoteDown, 0.0f);
            }
...
void MidiKeyboardComponent::mouseWheelMove (const MouseEvent&, const MouseWheelDetails& wheel) {
    const float amount = (orientation == horizontalKeyboard && wheel.deltaX != 0)
                             ? wheel.deltaX
                             : (orientation == verticalKeyboardFacingLeft ? wheel.deltaY
                                                                          : -wheel.deltaY);
