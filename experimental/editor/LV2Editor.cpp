...
void LV2Editor::port_event (uint32_t port, uint32_t size, uint32_t format, void const* buffer) {
    if (port == Port::AtomOutput && format == uris->atom_eventTransfer) {
        ...
        if (atom.type() == uris->atom_Object && atom.has_object_type (333333)) {
            ...
            lv2_atom_object_get (object, uris->patch_body, &body, uris->patch_request, &request, uris->patch_sequenceNumber, &sequenceNumber, 0);
...
const unsigned ksp1ui_x11  = KSP1::Gui::LV2Editor::register_class ("https://kushview.net/plugins/ksp1/ui/x11");
const unsigned ksp1ui_juce = KSP1::Gui::LV2Editor::register_class ("https://kushview.net/plugins/ksp1/ui/juce");
