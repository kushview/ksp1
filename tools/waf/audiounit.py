...
def options(opt):
    ...
    opt.add_option ('--core-audio-root', dest='core_audio_root', type='string', \
        help='Path To Core Audio SDK', default='')
...
def configure (conf):
    ...
    if not conf.options.core_audio_root:
        conf.options.core_audio_root = '/Developer/Extras/CoreAudio'

    conf.env.CORE_AUDIO_ROOT = os.path.join ('..', os.path.relpath (conf.options.core_audio_root, outdir))

    conf.env.AU_LIBDIR = '/Library/Audio/Plug-Ins/Components'
    if conf.options.apple_userdirs:
        conf.env.AU_LIBDIR = os.path.expanduser('~') + conf.env.AU_LIBDIR
...
def process_carbon_resource (self):
    ...
    if resource:
        ...
        name = apple.bundle_name_for_output (out,  getattr (self, 'mac_extension', '.component'))
        ...
        resdir = getattr (self, 'install_path', self.env.AU_LIBDIR) + '/%s/Contents/Resources' % name
...
class aurez (Task.Task):
    ...
    def run (self):
        cmd = [self.env.REZ, '-o', self.outputs[0].abspath(),
               '-arch', 'x86_64', '-useDF', '-script', 'Roman', '-d', 'x86_64_YES', '-d', 'SystemSevenOrLater=1',
               '-arch', 'i386', '-d', 'i386_YES',
