...
SamplerVoice::SamplerVoice (SamplerSynth& parent, int voiceId)
...
void SamplerVoice::startNote (const int midiNote,
    ...
    if (key.triggerMode == TriggerMode::Retrigger || key.voiceGroup >= 0) {
        for (int v = owner.getNumVoices(); --v >= 0;) {
            ...
            if (voice->sound && key.voiceGroup >= 0 && key.voiceGroup == voice->sound->getKeyInfo().voiceGroup) {
                voice->stopNote (0.0f, false);
            }
#endif
        }
    ...
    for (int i = sound->getNumLayers(); --i >= 0;) {
...
void SamplerVoice::_renderNextBlock (juce::AudioSampleBuffer& outputBuffer, int startSample, int numSamples) {
    if (sound == nullptr)
        return;
    ...
    const KeyInfo& key (sound->getKeyInfo());
    const int numLayers = sound->getNumLayers();
    ...
    for (int i = 0; i < numLayers; ++i) {
        ...
        while (renderFrame < numSamples) {
            if (frame + renderFrame >= layer->getStart()) {
                const int layerFrame = static_cast<int> (layerPosition[i]);
    ...
    const int numOuts = outputBuffer.getNumChannels();
    int renderChannel = 0;
    int gainIndex     = 0;
    float gain        = 1.0f;
    ...
    for (int chan = 2; chan < numOuts; ++chan) {
        ...
        outputBuffer.addFromWithRamp (chan, startSample, outputBuffer.getReadPointer (renderChannel, startSample), numSamples, lastGains[gainIndex], gain);
...
#define BAIL_RENDER     \
    ...
    while (--numSamples >= 0) {
        for (int i = 0; i < numLayers; ++i) {
            const int pos                = (int) layerPosition[i];
            const LayerData* const layer = sound->getLayer (i);

            // inclusively check velocity range 0.0 to 1.0
#if KSP1_USE_VELOCITY_RANGE
            if (! (layer->velocityRange.getStart() <= velocityNormal && velocityNormal <= layer->velocityRange.getEnd()))
                continue;
#endif
