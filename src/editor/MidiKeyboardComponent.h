/*
   ...
   * Modified by Kushview, LLC to allow special note finding during drag/drop
     operations
...
protected:
    ...
    */
    ...
    /** Finds the note for a given x and y coordinate.

        The normal getNoteAtPosition method yeilds unexpected results when calling during a
        drag operation as a DragAndDropTarget subclass

        @param pos The position to find the note under
    */
    int findNoteAt (Point<int> pos);
